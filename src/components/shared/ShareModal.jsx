...
const SHARE_OPTIONS = [
  ...
  },
  {
    ...
    getUrl: (url, title) => `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent("Check out this listing: " + url)}`,
  },
];
...
export default function ShareModal({ open, onClose, url, title }) {
  ...
  const handleShare = (option) => {
    ...
    const shareUrl = option.getUrl(url, title);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-xs p-0 overflow-hidden rounded-2xl">
        <DialogHeader className="px-5 pt-5 pb-3">
          <DialogTitle className="text-sm font-semibold text-gray-800">Share this listing</DialogTitle>
        </DialogHeader>
        ...
        <div className="mx-3 mb-4 flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
          <span className="flex-1 text-xs text-gray-400 truncate">{url}</span>
          <button
            onClick={handleCopy}
            className="shrink-0 w-7 h-7 rounded-lg bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-100 transition-colors shadow-sm"
          >
